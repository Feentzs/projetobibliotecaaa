...
function createCard(book) {
  ...
  // Event listener para o botão "Ler agora"
...
function initSearch() {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => handleSearch(e.target.value, heroSearchOverlay, searchBackdrop));
    searchInput.addEventListener('focus', () => showSearchOverlay(heroSearchOverlay, searchBackdrop));
  }

  if (headerSearchInput) {
    headerSearchInput.addEventListener('input', (e) => handleSearch(e.target.value, headerSearchOverlay, searchBackdrop));
...
function handleSearch(query, overlay, backdrop) {
  const q = query.trim().toLowerCase();
  
  // Usar um Set para evitar duplicatas
  const booksSet = new Set();
  const allBooksArray = [...recommended, ...library, ...topRated, ...newReleases, ...popularNow];
  
  allBooksArray.forEach(b => booksSet.add(JSON.stringify(b)));
  const uniqueBooks = Array.from(booksSet).map(str => JSON.parse(str));
  ...
  const results = uniqueBooks.filter(b =>
...
function renderSearchResults(results, overlay) {
  ...
  const resultsHTML = results.slice(0, 8).map(book => `
    <a href="#" class="search-result-item" data-book-id="${book.id}">
      <img src="${book.cover_url || book.cover}" alt="${book.title}" />
